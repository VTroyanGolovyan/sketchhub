function Ellips(state, ctx){
  ...
  this.render = function(ctx){
    if (this.active){
       ...
       ctx.beginPath();
       ctx.ellipse(Math.floor((this.x + this.startvX)/2),Math.floor((this.y+this.startvY)/2), Math.abs(Math.floor((this.x - this.startvX)/2)-d),  Math.abs(Math.floor((this.y - this.startvY)/2)-d),0,0,2 * Math.PI);
       ctx.stroke();
    }
  }
  ...
  this.onmouseup  = function(coords){
     ...
     if(width > 0 && height > 0){
      ...
      // ctx.strokeRect(d,d,l.width-2*d,l.height-2*d);
       ...
       ctx.ellipse(Math.floor(l.width/2),Math.floor(l.height/2), Math.abs(Math.floor(l.width/2)-d),Math.abs(Math.floor(l.height/2)-d),0,0,2 * Math.PI);
       ctx.stroke();
